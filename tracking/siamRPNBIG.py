...
from data_loader import TrackerRGBTDataLoader
...
class TrackerSiamRPNBIG(Tracker):
    ...
    def init(self, exemplar_rgb_img, exemplar_ir_img, bbox):
        ...
        exemplar_rgb_img, _, _ = self.data_loader.get_exemplar_image(   exemplar_rgb_img,
                                                                        self.bbox,
    ...
    def update(self, instance_rgb_img, instance_ir_img):
        ...
        """
        instance_rgb_img = instance_rgb_img
        instance_ir_img = instance_ir_img
        frame = instance_rgb_img
        #cv2.imshow('instance_img', instance_ir_img)
        self.img_mean = np.mean(instance_rgb_img, axis=(0, 1))

        instance_rgb_img, _, _, _ = self.data_loader.get_instance_image(   instance_rgb_img,
                                                                           self.bbox,
                                                                           config.template_img_size,
        ...
        instance_ir_img, _, _, scale_x = self.data_loader.get_instance_image(   instance_ir_img,
                                                                                self.bbox,
                                                                                config.template_img_size,
                                                                                config.detection_img_size,
                                                                                config.context_amount,
