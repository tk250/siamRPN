...
class TrackerDataLoader_ir(Dataset):
    ...
    def open(self):
        ...
        template_img = Image.open(self.ret['template_img_path']).convert('L')
        template_img = np.array(template_img)

        detection_img = Image.open(self.ret['detection_img_path']).convert('L')
        ...
        img_mean_d = int(detection_img.mean())
    ...
    def crop_and_pad(self, img, cx, cy, gt_w, gt_h, a_x, b_y, model_sz, original_sz, img_mean=None):
        ...
        if any([top, bottom, left, right]):
            te_im_ = np.zeros((int((r + top + bottom)), int((c + left + right))), np.uint8)  # 0 is better than 1 initialization
